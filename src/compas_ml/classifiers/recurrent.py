...
def recurrent(training_data, training_labels, testing_data, testing_labels, classes, neurons, steps, batch,
    ...
    ----------
    training_data : list
        Training data of length m, each entry is a list of length n.
    training_labels : list
        Training labels of length m.
    testing_data : list
        Testing data of length p, each entry is a list of length n.
    testing_labels : list
    ...
    def make_batch(batch_size, data, labels, lengths):
        ...
        x_ = [[word_index[word] for word in data[i]] for i in batch]
    ...
    train_   = tf.train.RMSPropOptimizer(0.001, 0.9).minimize(loss)
    ...
    with tf.Session() as session:
        ...
        for i in range(steps):
            ...
            session.run(train_, feed_dict={x: x_batch, y: y_batch, z: z_batch})
...
if __name__ == "__main__":
    ...
    training_data   = strings_from_csv('{0}training_data.csv'.format(path))
    testing_data    = strings_from_csv('{0}testing_data.csv'.format(path))
    training_labels = integers_from_csv('{0}training_labels.csv'.format(path))
    testing_labels  = integers_from_csv('{0}testing_labels.csv'.format(path))
