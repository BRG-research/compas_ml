...
if __name__ == "__main__":
    ...
    nce_weights = tf.Variable(tf.truncated_normal([vocabulary_size, embedding_dimension],
                                                  stddev=1 / math.sqrt(embedding_dimension)))
    ...
    learningRate = tf.train.exponential_decay(learning_rate=0.1, global_step=global_step, decay_steps=1000,
                                              decay_rate=0.95, staircase=True)
    train_step = tf.train.GradientDescentOptimizer(learningRate).minimize(loss)  # add to all
    ...
    with tf.Session() as sess:
        ...
        for step in range(1000):
            ...
            if step % 100 == 0:
                saver.save(sess, os.path.join(log_dir, 'w2v.ckpt'), step)
                loss_value = sess.run(loss, feed_dict={train_inputs: x_batch, train_labels: y_batch})
                print('Loss {0} {1}'.format(step, loss_value))
